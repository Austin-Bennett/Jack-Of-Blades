...
GENERATE_DYNAMIC_LEVEL_OBJECT(sprite)
    protected:
    float health;
    float max_health;
...
public:
    ...
    static void drawShadow(dvec2 pos, float w, float h) {
        DrawEllipse(EXPAND_V(pos.convert_data<i32>()), w, h, Fade(BLACK, 0.2));
    }
};


struct spriteSpawnPoint : LevelObject {
    GENERATE_LEVEL_OBJECT(spriteSpawnPoint);


    spriteSpawnPoint() : LevelObject(rect(0, 0, 32, 32), collisionType::NO_COLLISION, true) {

    }

    explicit spriteSpawnPoint(dvec2 pos) : LevelObject(rect(pos, 32, 32), collisionType::NO_COLLISION, true) {}
    ...
    shared_ptr<LevelObject> copy(dvec2 pos) override {
        return make_shared<spriteSpawnPoint>(collision.pure().pos(pos).pos());
    }
...
struct spriteSpawnPointFactory {
    ...
    NODISCARD static json objectToJson(const LevelObject& x) {
        json ret;
        ret["position"] = vector{EXPAND_V(x.getCollision().pos())};
        return ret;
    }
...
#define GENERATE_SPAWN_POINT(SPRITE_TYPE, ID) struct SPRITE_TYPE##SpawnPoint : spriteSpawnPoint {\
                                                GENERATE_LEVEL_OBJECT(SPRITE_TYPE##SpawnPoint)\
                                                ...
                                                shared_ptr<LevelObject> copy(dvec2 pos) override {\
                                                    return make_shared<SPRITE_TYPE##SpawnPoint>(collision.pure().pos(pos).pos());\
                                                }\
                                          };\
